...
Sometimes, we just want to analyze particular element of the data. To extract the informations and still make the data tidy, we can reshape our data according to the observations of interest. Let's try data transformations with two `tidyr` functions `spread` and `gather`.
...
We could now plot comparisons between the weight of species in different plots, although we may wish to fill in the missing values first. For example, let's fill the empty value with 0.
